...
async def ingest_once():
    """One sweep of the watched boxes. Scheduled from backend/ingest."""
    ...
    try:
        async with httpx.AsyncClient(timeout=_TIMEOUT) as client:
            for index, box in enumerate(boxes):
                if index:
                    # Paced rather than concurrent. Five requests a minute is
                    # the whole budget, and a sweep that spends it in one burst
                    # is how a free tier becomes a blocked one.
                    await asyncio.sleep(REQUEST_SPACING)
                try:
                    entries = await _fetch_box(client, box)
                except Exception as exc:  # noqa: BLE001 - one box is not the sweep
                    # Recorded and carried, not raised: a box that fails leaves
                    # the other seven perfectly good, and a chokepoint quietly
                    # missing from the layer is worse than a partial success
                    # that says which one.
                    failures.append(f"{box}: {type(exc).__name__}")
                    log.warning("Marinesia box %s failed: %s", box, exc)
                    continue
                for entry in entries:
                    record = parse_vessel(entry, now)
                    if record is None:
                        continue
                    # Boxes overlap, and a hull in two of them should be one
                    # pin. Newest fix wins.
        ...
        if not records and failures:
            raise RuntimeError(f"every box failed: {'; '.join(failures)}")

        state.data = records
        state.last_success = time.time()
        state.last_error = f"partial sweep -- {'; '.join(failures)}" if failures else None
        await storage.record_snapshot(SOURCE, records, "mmsi")
        await storage.record_source_health(SOURCE, len(records), True, state.last_error)
        log.info(
            "Marinesia: %d vessels across %d/%d boxes (%s, %d sanctioned)",
            len(records), len(boxes) - len(failures), len(boxes),
        ...
        log.warning("Marinesia sweep failed: %s", exc)
        await storage.record_source_health(SOURCE, None, False, str(exc))
